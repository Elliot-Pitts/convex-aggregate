...
async function validateAndSetSumType(db, tree, summand) {
    ...
    if (tree.sumType !== incomingType) {
        ...
        throw new ConvexError(`Aggregate type mismatch: this aggregate uses ${expected} but received ${received}. ` +
...
async function validateNode(ctx, namespace, node, depth) {
    ...
    if (typeof acc.sum === "number" && typeof nAggregate.sum === "number") {
        ...
        if (accKeys.length !== nKeys.length || !accKeys.every(k => nKeys.includes(k))) {
            throw new ConvexError(`node ${node} sum keys do not match subtrees ${JSON.stringify(acc.sum)} !== ${JSON.stringify(nAggregate.sum)}`);
        }
        for (const key of accKeys) {
            if (Math.abs(acc.sum[key] - nAggregate.sum[key]) > 0.0001) {
                throw new ConvexError(`node ${node} sum[${key}] does not match subtrees ${acc.sum[key]} !== ${nAggregate.sum[key]}`);
            }
        }
    }
    else {
        throw new ConvexError(`node ${node} sum types do not match: ${typeof acc.sum} !== ${typeof nAggregate.sum}`);
    }
            ...
            if (typeof totalSum === "number" && typeof newN.aggregate.sum === "number") {
                ...
                for (const key of Object.keys(totalSum)) {
                    if (Math.abs(totalSum[key] - (newN.aggregate.sum[key] ?? 0)) > 0.00001) {
                        throw new Error(`bad sum split for ${key}: ${totalSum[key]} !== ${newN.aggregate.sum[key]}`);
                    }
                }
            }
        }
